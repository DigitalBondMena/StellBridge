...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, SharedServiceCardComponent],
...
export class ServicesComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  activeHorizontalItem = signal<number | null>(null);
  ...
  trackByAccordionId = (index: number, item: AccordionItem): number => item.id;
  ...
  private initializeAccordionItems(): void {
    ...
    const items: AccordionItem[] = services!.map((service, index) => ({
      id: index, // Use index as id since services don't have id property
      isActive: false,
      isOpen: false,
    }));
   ...
   */
  onAccordionItemClick(serviceId: number, event: Event): void {
   ...
   */
  onHorizontalItemMouseEnter(serviceId: number): void {
    ...
    allItems.forEach((item: Element) => {
      const htmlItem = item as HTMLElement;
      const itemId = parseInt(htmlItem.getAttribute('data-service-id') || '0');
   ...
   */
  private slideDown(serviceId: number): void {
   ...
   */
  private slideUp(serviceId: number): void {
    const element = this.elementRef.nativeElement.querySelector(
      `[data-service-id="${serviceId}"] .tt-accordion-content`
    ) as HTMLElement;
    if (!element) return;
   ...
   */
  private getItemIdFromElement(element: HTMLElement): number | null {
   ...
   */
  isAccordionItemActive(serviceId: number): boolean {
    return (
   ...
   */
  isHorizontalItemActive(serviceId: number): boolean {
    return (
